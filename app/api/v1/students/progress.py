"""
Student learning activities and progress tracking using Supabase backend
"""
import logging
...
from app.models import ConceptProgressUpdate, StudentProgressResponse, LearningActivityRequest
        ...
        if "@" in student_id:
            try:
                ...
                existing_by_email = next(
                    (s for s in students if s.email == student_id), None)
                if existing_by_email:
                    resolved_student_id = existing_by_email.student_id
                    logger.info(
                        f"Found existing student by email: {resolved_student_id}")
                else:
                ...
                logger.warning(
                    f"Could not check existing students: {str(email_check_error)}")
