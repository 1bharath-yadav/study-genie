...
from app.llm.langchain import get_llm_response, get_llm_response_stream
...
async def process_files_stream_endpoint(
    ...
    """
    Stream structured learning content back to the client as Server-Sent Events (SSE).
    Each SSE data event contains a JSON-encoded partial validated chunk from Pydantic-AI.
    """
    # Resolve model preference and api key same as non-stream endpoint
    student_id = current_user["sub"]
    preferences = get_user_model_preferences(student_id)
    preferred_model = None
    for pref in preferences:
    ...
    try:
        ...
        # Create an async generator that yields SSE encoded partials
        async def event_generator():
            try:
                async for partial in get_llm_response_stream(
                    uploaded_files_paths=[Path(p) for p in saved_paths],
                    userprompt=user_query or "Please analyze this content and create study materials.",
                    temp_dir=tmp_dir,
                    user_api_key=api_key,
                ...
                logger.exception("Error while streaming LLM response: %s", e)
                ...
                try:
                    ...
                    logger.debug("Error during cleanup after streaming", exc_info=True)
